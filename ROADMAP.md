...
- [ ] Run each config for a short pilot budget (recommended: 250M tokens each).
   ...
   - [ ] Execute the full pilot sweep on target GPU(s) and collect per-config logs/artifacts.
      ...
      - [x] Add pilot sweep launch-script artifact output (`--output-launch-script-sh`) that emits canonical per-config `scripts.pilot_sweep --target ...` commands (including expected log/metrics artifact paths and optional resume mode) so target-GPU operators can execute/shard Stage 1 runs without manual command assembly.
      - [x] Add resumable sweep support (`--resume-from-artifacts`) so interrupted long pilot runs can continue from existing per-config JSON artifacts.
        - [ ] Use resume mode for the real target-GPU sweep and persist final ranking/finalist artifacts.
              ...
              - [x] Add strict resume-artifact gating in `scripts.pilot_sweep.py` (`--require-resume-artifacts`) so full-grid resume/check-in runs fail fast when any expected per-config log/metrics pair is missing.
